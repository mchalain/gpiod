...
defconfig: $(builddir)$(CONFIG).old cleanconfig FORCE
	@echo "  "DEFCONFIG $*
	@$(if $(wildcard $(DEFCONFIG)), $(GREP) -v "^#" $(DEFCONFIG) > $(builddir)$(CONFIG), touch $(builddir)$(CONFIG))
...
CONFIGS:=$(if $(wildcard $(DEFCONFIG)), $(shell cat $(DEFCONFIG) | sed 's/\"/\\\"/g' | grep -v '^\#' | awk -F= 't$$1 != t {print $$1}' ))
oldconfig: $(wildcard $(DEFCONFIG)) FORCE
	@echo "  "OLDCONFIG
